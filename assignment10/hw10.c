...
// is the set represented by boolean array s of size "size" not empty? empty
// means all elements are false
...
// return the row of the table (with "size" rows) with the minimum distance
// field - only use valid rows
...
// find all the neighbors of v in graph g and set those values to true in array
// "neighbors" ("size" number of nodes) note that contents of array neighbors
...
int main() {
  ...
  while (setNotEmpty(Q, GSIZE)) {
    ...
    // ********** INSERT YOUR CODE HERE **********
    Q[u] = false;
    // DIJKSTRA: for each neighbor v of u (ignore vertices that we have already
    // finished)
    for (v = 0; v < GSIZE; v++) {
      if (S[v] && Q[v]) {
